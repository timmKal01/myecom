<x-dashboard-layout role="admin" active="reviews" title="Product Reviews">
    <div class="bg-surface border border-line rounded-xl shadow-sm p-12 text-center">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 mx-auto mb-4 text-ink-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 21.539a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.562.562 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
        </svg>
        <h2 class="font-display text-lg font-semibold mb-2">No review system yet</h2>
        <p class="text-sm text-ink-muted max-w-sm mx-auto">
            Customer product reviews aren't collected yet — there's no rating field on checkout or the product page. This page is ready to wire up once that's built.
        </p>
    </div>
</x-dashboard-layout>
