<x-admin-layout active="subcategory" title="Add Subcategory">
    <div class="max-w-xl">
        <div class="bg-admin-surface border border-admin-border rounded-2xl p-6 sm:p-8">
            <h2 class="text-lg font-semibold mb-6">New Subcategory</h2>

            @if ($categories->isEmpty())
                <p class="text-sm text-admin-ink-muted">
                    You need at least one category before you can add a subcategory.
                    <a href="{{ route('category.create') }}" class="text-admin-accent font-medium hover:underline">Add a category</a>.
                </p>
            @else
